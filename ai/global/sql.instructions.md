...
- **P1.** **Baseline**: run the example EXEC with `SET STATISTICS IO/TIME ON`; see [sql.examples.md](sql.examples.md).
- **P2.** **Identify hotspots**: high logical reads on large tables; prefer index seeks over table scans.
- **P3.** **Optimise** (minimum set):
...
- **P4.** **No query hints**: avoid `OPTION (...)`, `FORCE ORDER`, `USE PLAN`, or index hints unless measured proof justifies them.
- **P5.** **Linter constraint (SRP0009)**: build rules block `DATEPART()` / function calls on columns in `WHERE` clauses; use `DECLARE` variables to pre-compute values outside the query.
- **P6.** **Record**: update `docs/Benchmarks/<Schema>.md` with before/after logical-read and Workfile totals for material changes, including example parameters.
- **P7.** **Known Bottlenecks**: if a performance issue cannot be fixed immediately, document it as a "Known Bottleneck" in `docs/Benchmarks/<Schema>.md` and raise a GitHub issue with the bottleneck details, affected procedure/view, observed stats, and proposed fix. Reference the issue number in the benchmark file.
